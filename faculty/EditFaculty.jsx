...
function EditFaculty() {
  ...
  function getFacultyToEdit(){
    axios.get(`http://localhost:3000/faculty/${id}`)
          .then(res=>{
            if(res.status===200){
             setValue('facultyPhoto' , res.data.facultyPhoto);
             setValue('facultyId' , res.data.facultyId);
            setValue('facultyName', res.data.facultyName)
            setValue('facultyCourse',res.data.facultyCourse)
            setValue('facultyExperience',res.data.facultyExperience)
            setValue('facultyContact',res.data.facultyContact)            
            }
          })
  }
  useEffect(()=>getFacultyToEdit,[])
   ...
   function updateFaculty(data){
    console.log(data)
    axios.put( `http://localhost:3000/faculty/${id}`,data)
    .then(res=>{
      if(res.status===200){
  ...
  return (
    <div className="container-fluid d-flex justify-content-center align-items-center  m-5" style={{ minHeight: "100vh"  }}>
      ...
      <div className="text-center mb-4">
        <h2 className="fw-bold text-primary"> Update Faculty Details</h2>
        <p className="text-muted">Fill the details carefully</p>
      </div>

      <form onSubmit={handleSubmit(updateFaculty)}>
